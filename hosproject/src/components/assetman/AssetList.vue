  ...
  <div class="assetlist">
    ...
    <div class="banner">
      <div class="bannerleft">
        ...
        <el-button type="warning" plain @click="loadTem">模板下载</el-button>
        <input @change="upImage"  type="file" name="file" class="el-upload__input" style="display: none">
        <el-button type="warning" plain @click="clickUpload">导入</el-button>
        <el-button type="warning" plain @click="exportHandle">导出</el-button>
        <el-button type="warning" plain @click="printHandle">标签打印</el-button>
      </div>
    ...
    <div class="table center-table">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tag
              ...
              disable-transitions>{{scope.row.useDepartmentName?'已领用':'闲置'}}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          label="操作">
          <template slot-scope="scope">
            <span style="color: #409eff;cursor: pointer" v-if="(!scope.row.useDepartmentName)"
                  @click="changeClick(scope.row)" type="primary" size="mini">编辑</span>
            <span style="color: #e6a23c;cursor: pointer" v-if="(scope.row.useDepartmentName)" type="info" size="mini"
                  @click="returnback(scope.row)">退还</span>
            <span style="color: #f56c6c;cursor: pointer" type="info" size="mini"
                  @click="baofeiClick(scope.row)">报废</span>
          </template>
        </el-table-column>
      </el-table>
    ...
    <mask-shadow style="z-index:99;" v-if="showGoodsDetail">
      <goods-detail :useEditShow="useEditShow" @quickhandle="quickhandle" :showedit="showedit" :uersdata='uersdata'
                    :uersdatack='uersdatack' @close="closeGoodsDetailCom" @editClickpose="editClickpose"></goods-detail>
    </mask-shadow>
    <mask-shadow style="z-index:999;" v-if="addShow">
      <add-assetlist :officeListCommon="officeListCommon" :areaListCommon="areaListCommon"
                     :typeListCommon="typeListCommon" :edititem="edititem" :editadd="editadd" :areaList="areaList"
                     @close="closeGoodsDetail" :officeList="officeList" :personList="personList" :adminList="adminList"
                     :typeList="typeList" @addSure="addSure"></add-assetlist>
    </mask-shadow>
    <mask-shadow v-if="recipientsPage" style="z-index:999">
      <recipients :isDisabledInfo="isDisabledInfo" :type="type" @recipentSure="recipentSure" :areaList="areaList"
                  :personList="personList" @close="closeGoodsDetail" :lingyongList="lingyongList"
                  :officeList="officeList"></recipients>
    </mask-shadow>
    ...
    <mask-shadow style="z-index:999" v-if="baofeiShow">
      <baofei :scapeReasonList="scapeReasonList" :isDisabledInfo="isDisabledInfo" @close="closeGoodsDetail"
              :witchone="witchone"></baofei>
    </mask-shadow>
    <mask-shadow style="z-index:999" v-if="tuihuanShow">
      <tuihuan :isDisabledInfo="isDisabledInfo" @close="closeGoodsDetail" :witchone="witchone"
               :areaList="areaList"></tuihuan>
    </mask-shadow>
  ...
  export default {
    ...
    methods: {
      printHandle(){
        ...
        let param = __PATH.COMMONPATH+'assetsInfo/exportAssetsCodePdf'
        let params = {}
      ...
      upImage(e){
        ...
        vm.$http.post('assetsInfo/importData',{
          file:_file,
          flagkuayu:true
        }).then(res=>{
      ...
      loadTem(){
        let vm =this
        let param = __PATH.COMMONPATH+'assetsInfo/importData/template'
        let params = {}
        var obj = JSON.parse(localStorage.getItem('LOGINDATA'))
        params.unitCode = obj.unitCode
        params.hospitalCode = obj.hospitalCode
      ...
      requestScape() {
        let vm = this
        vm.$http.post('dictUtils/getDictList', {
      ...
      recipientsList: function (type) {
        ...
        let _i = vm.selectData.findIndex(function (item) {
          return item.status == '已领用'
        })
        if (type == 0 && _i > -1) {
          vm.$message({
            message: '只能领用闲置状态下的资产',
            type: 'warning'
          });
          return
        }
